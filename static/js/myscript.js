...
//userAgent in IE11 Win7 returns: Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko
	...
	jQuery(function(){
		...
		if ( windowWidth <= 900 ){
			...
			jQuery('.section').width(jQuery(window).width());
			} else {
			*/			
			var anchors = ["home", "callejas-asociados", "practice-area", "law-firm", "contact"];
		...
		jQuery(document).ready(function(){
			if(jQuery(window).width() < 1008){
				jQuery('#fullpage > div').removeClass('fp-section');
				jQuery('.front .fullpage .section').width(jQuery(window).width());
				//jQuery('.front .fullpage .section').height(jQuery(window).height());
				jQuery('.front .fullpage .section').css('height', 'auto', 'important');
				jQuery('.section').css('position', 'relative');
			}else {
				jQuery('#fullpage > div').addClass('fp-section');	
			}
		})
		
		jQuery(window).resize(function(){
			if(jQuery(window).width() < 1008){
				jQuery('.front div').removeClass('fp-section');
				jQuery('.front .fullpage .section').width(jQuery(window).width());
				//jQuery('.front .fullpage .section').height(jQuery(window).height());
				jQuery('.front .fullpage .section').css('height', 'auto', 'important');
				jQuery('.section').css('position', 'relative');
				
				jQuery('.section div').removeClass('region-fixed');
	...
	jQuery(function(){
		...
		var height = jQuery('#section0').height();
			var height2 = 3*height;
			var height3 = 2*height;
			var height4 = 3*height;
			  jQuery(window).scroll(function() {
				var scroll = getCurrentScroll();
				  if (( scroll >= height) && (scroll <= height2))   {
				
					   jQuery('.region-section1').addClass('region-fixed');
					   jQuery('.horea-sing').addClass('region-fixed');
					    jQuery('#section1').addClass('fixat');
					   jQuery('#section3').addClass('fixat');
					}
					else {
						jQuery('.region-section1').removeClass('region-fixed');
						jQuery('.horea-sing').removeClass('region-fixed');
						jQuery('#section3').removeClass('fixat');
						jQuery('#section1').removeClass('fixat');
					}
					
					 if (( scroll >= height3) && (scroll < height4))   {
						  jQuery('.region-section3').addClass('region-fixed');		
							jQuery('.horea-sing2').addClass('region-fixed');							  
					 }
					 else {
						  jQuery('.region-section3').removeClass('region-fixed');	
						  jQuery('.horea-sing2').removeClass('region-fixed');							  
			...
			function getCurrentScroll() {
			 ...
			 jQuery('.scroll').addClass('absolute');
			  jQuery('.region-section0 #block-block-1').addClass('absolute');
		}
	...
	jQuery(function(){
	   ...
	   var navHeight = jQuery( window ).height() - 80;
